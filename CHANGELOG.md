...
### Changed
- Enabled Molten Core as a tracked raid (Settings tab zone checklist and the ranking scan) - was previously listed but greyed out pending confirmation it's live on this server. Hydraxian Waterlords reputation (earned in Molten Core) stays excluded for now by deliberate choice, not tied to Molten Core's own availability anymore.

<details>
<summary>v1.26.63 - 2026-08-03</summary>
...
- Improved loot window and need/greed roll popups showing a scaled item's pre-settle stats (and score) instead of its real, correct value - reading straight off the already-displayed tooltip (the fix above) is correct for an item you already own, but a not-yet-looted item is a different case. Hovering a loot slot or roll popup now goes back to independently querying the item's current value on every hover, same as the background ranking scan already did, instead of reading whatever the tooltip happens to be showing at that instant. **Known limitation:** a Need/Greed/Pass roll popup specifically can still show pre-settle stats for the entire duration of the roll, even though the scaled value is personal to your own level (not dependent on who wins) - the correct value doesn't appear to become available from the client until the item is actually looted, regardless of how or how often it's queried. Treat a roll-popup score as provisional until you've actually won and looted the item.
...
- Fixed severe, dungeon-noticeable lag from looting - the automatic dungeon/raid/reputation ranking rescan (fires ~5 seconds after any equipment change, i.e. shortly after looting and equipping an upgrade) and the Instance Loot tab's own zone scan both score potentially hundreds or thousands of hypothetical items (from AtlasLoot's static loot-table database, not real dropped instances) and were each paying for a real tooltip-scan query (a server round-trip, often for an item this client had never cached before) per item, as a side effect of this version's scaling-correction work - previously a purely local, instant lookup. Neither scan is looking at one specific real item in front of the player; both are a rough "worth surfacing" pass over many items, where a slightly-off scaled-item estimate is a fine tradeoff against a client freeze. Both now skip the scaling-correction query entirely and score straight off GetItemStats(). The same skip also applies to the (Settings-gated, off by default) upgrade-glow check on loot/roll/vendor/quest-reward items, for anyone with that enabled.
- Fixed the Instance Loot tab re-running its entire zone scan from scratch on every GET_ITEM_INFO_RECEIVED event, with no debounce and no guard against a second scan starting while an earlier one was still mid-batch - harmless in isolation, but this event can arrive in rapid bursts (several items resolving within the same second, common right after a loading screen or looting several corpses back to back) while the tab happens to be open, stacking up several full, overlapping rescans. Now debounced to a single rescan once a burst of these settles.
