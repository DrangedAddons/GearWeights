...
- Fixed severe, dungeon-noticeable lag when looting, most reliably on Need/Greed/Pass rolls and loot windows containing a "world drop" item (anything not on AtlasLoot's own recognized boss-loot list). Deciding whether to glow an item as an upgrade (loot window, roll popup, vendor, quest reward) scores every item shown, synchronously, with no batching between them - fine when that was a purely local, instant lookup, but every one of those items was now also paying for a real tooltip-scan query (a server round-trip, often for an item this client had never cached before) as a side effect of this version's scaling-correction work. A boss or pull dropping several such items at once meant several of those queries back to back in a single frame. Glow checks don't need scaling precision - they're a rough "worth highlighting" signal, and the actual tooltip hover afterward still gives the real, corrected score - so they now skip the scaling check entirely and score straight off GetItemStats(). The same skip was applied to the dungeon/raid/reputation ranking scans (Settings tab and Instance Loot tab), which score potentially thousands of hypothetical items from AtlasLoot's database and had the same new cost multiplied across all of them.
